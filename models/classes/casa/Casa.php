...
class casa
{
    ...
    public function selectCasesCerca($dataInici, $dataFi, $idioma)
    {
        ...
        $query = "SELECT casa.id, traduccioCasa.traduccioNom, traduccioCasa.tradDescripcio, imatge.img_principal, casa.nBanys, casa.nHabitacions FROM casa JOIN traduccioCasa ON casa.id = traduccioCasa.casa_id JOIN imatge ON casa.id = imatge.casa_id WHERE traduccioCasa.idioma_id=? AND casa.id NOT IN (SELECT casa_id FROM reserva WHERE NOT (reserva.id = 0) AND ((?) BETWEEN data_inici AND data_fi ) OR ((?) BETWEEN data_inici AND data_fi ) OR ( data_inici BETWEEN (?) AND (?) ))";
    ...
    {
        $query = "SELECT poblacio.nom, traduccioCasa.traduccioNom,traduccioCasa.tradDescripcio,reserva.casa_id,  DATE_FORMAT(reserva.data_inici,'%d-%m-%Y') as entrada,  DATE_FORMAT(reserva.data_fi,'%d-%m-%Y') as sortida, reserva.data_reserva, reserva.preu_final, reserva.data_inici from reserva, usuari, casa, traduccioCasa, poblacio WHERE NOT (reserva.id = 0) AND reserva.usuari_id = usuari.id AND casa.id = traduccioCasa.casa_id AND casa.id = reserva.casa_id AND poblacio.id = casa.poblacio_id AND usuari.token = ? AND traduccioCasa.idioma_id = ? AND reserva.data_inici < ? ORDER BY reserva.data_reserva desc";
        $stmt = $this->conexio->prepare($query);
    ...
    public function insertReserva($id_usuari, $data_inici, $data_fi, $data_reserva, $preu_final, $codiPag)
    {
        $query = "INSERT INTO reserva (casa_id, usuari_id, data_inici, data_fi, data_reserva, preu_final, id) VALUES (?, ?, ?, ?, ?, ?, ?);";

        $stmt = $this->conexio->prepare($query);
        $stmt->bind_param("iisssdi", $this->id, $id_usuari, $data_inici, $data_fi, $data_reserva, $preu_final, $codiPag);
        $stmt->execute();
